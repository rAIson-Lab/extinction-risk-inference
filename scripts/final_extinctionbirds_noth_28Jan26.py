...
# GUARDAR BASELINE
os.makedirs('confold_results_noth50pruning', exist_ok=True)
with open('confold_results_noth50pruning/01_baseline.txt', 'w') as f:
    baseline_model.asp()
...
with open('confold_results_noth50pruning/02_expert_with_confidence.txt', 'w') as f:
    expert_model.asp()
...
# LEARNED CONFIDENCE
with open('confold_results_noth50pruning/03_expert_learned_confidence.txt', 'w') as f:
...
with open('confold_results_noth50pruning/04_simple_pruned_model_expert_confidence.txt', 'w') as f:
...
print("--- Training with confidence_fit(improvement_threshold=0.5) ---")
advanced_pruning_model.confidence_fit(train_data, improvement_threshold=0.5)
...
# PRUNED MODEL
with open('confold_results_noth50pruning/05_advanced_pruning_model.txt', 'w') as f:
...
expert_advanced_model.confidence_fit(train_data, improvement_threshold=0.5)
...
with open('confold_results_noth50pruning/06_expert_advanced_pruning_model.txt', 'w') as f:
    ...
    f.write("Applied confidence_fit(improvement_threshold=0.5) for additional rules\n\n")
    f.write("RULES:\n" + "-"*70 + "\n")
